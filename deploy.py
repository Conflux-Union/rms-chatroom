...
# Only these directories are included in deployment package (whitelist)
DEPLOY_DIRS = [
    "backend",
    "packages/web/dist",
]

# Files/patterns to exclude within DEPLOY_DIRS
EXCLUDE_PATTERNS = {
    ".venv",
    "__pycache__",
    "*.pyc",
...
def build_frontend() -> tuple[bool, str]:
    ...
    try:
        ...
        if not WEB_DIST_DIR.exists():
            return False, f"Build succeeded but dist not found: {WEB_DIST_DIR}"
...
'''
    # Web version.ts
    WEB_VERSION_FILE.parent.mkdir(parents=True, exist_ok=True)
    WEB_VERSION_FILE.write_text(frontend_content)

    # Electron renderer version.ts
    ELECTRON_VERSION_FILE.parent.mkdir(parents=True, exist_ok=True)
    ELECTRON_VERSION_FILE.write_text(frontend_content)
...
"""
    ...
    if ELECTRON_VERSION_FILE.exists():
        ELECTRON_VERSION_FILE.write_text(default_frontend)
    BACKEND_VERSION_FILE.write_text(default_backend)


def create_archive() -> tuple[io.BytesIO, int, int]:
    """Create tar.gz archive of deployment files (whitelist mode)."""
    buffer = io.BytesIO()
    file_count = 0

    with tarfile.open(fileobj=buffer, mode="w:gz") as tar:
        for deploy_dir in DEPLOY_DIRS:
            dir_path = PROJECT_ROOT / deploy_dir
...
def main():
    ...
    print(f"      Generated packages/web/src/version.ts")
    print(f"      Generated packages/electron-renderer/src/version.ts")
    ...
    try:
        ...
        if not success:
            print(f"      ERROR: {err}")
            sys.exit(1)
        print(f"      Web build successful")
        print(f"      Output: packages/web/dist")

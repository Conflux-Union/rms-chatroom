...
- **Music Playback**: Client-side direct playback architecture
  ...
  - Credential auto-renewal: background loop in `handler/music.go` (hourly check); QQ musickey refreshed via `music.login.LoginServer.Login` with stored refresh_key/refresh_token when within 24h of expiry; NetEase session kept alive daily via `login/token/refresh`, rotated cookies re-persisted to disk on every change
